...
class HumanPlayer(Player):
  ...
  def get_move(self, game):
    ...
    while not valid_square:
      ...
      # we're goning to check that this is a correct value by trying to cast
      # it to an integer, and if it's not, then we say its invalid
      # if that spot is not availabe on the board, we also say its invalid
      try:
        val = int(square)
        if int(square) not in game.available_moves():
          raise ValueError
...
class GeniusComputerPlayer(Player):
  ...
  def minimax(self, state, player):
    ...
    for possible_move in state.available_moves():
      ...
      # step 4: update the dictionareis if necessary
